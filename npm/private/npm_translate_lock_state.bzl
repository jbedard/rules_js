...
################################################################################
def _init(priv, rctx, rctx_name, attr, label_store):
...
""".format(rctx_name = rctx_name))
    ...
    _init_patches_labels(priv, attr, label_store)
    ...
    if _should_update_pnpm_lock(priv) or not attr.pnpm_lock:
        ...
        _init_pnpm_labels(label_store, rctx, attr)

    if _should_update_pnpm_lock(priv):
        # labels only needed when updating the pnpm lock file
        _init_update_labels(priv, rctx_name, attr, label_store)
    ...
    if _should_update_pnpm_lock(priv):
        ...
        _copy_unspecified_input_files(priv, rctx, rctx_name, attr, label_store)

def _reload_lockfile(priv, rctx, rctx_name, attr, label_store):
    _load_lockfile(priv, rctx, attr, label_store)
...
################################################################################
def _init_pnpm_labels(label_store, rctx, attr):
    ...
    label_store.add("pnpm_entry", attr.use_pnpm if attr.use_pnpm else Label("@pnpm//:package/bin/pnpm.cjs"))

################################################################################
def _init_update_labels(priv, rctx_name, attr, label_store):
    ...
    action_cache_path = paths.join(
        ...
        PNPM_LOCK_ACTION_CACHE_PREFIX + base64.encode(utils.hash(helpers.to_apparent_repo_name(rctx_name) + pnpm_lock_label_str)),
    )
...
################################################################################
def _init_patches_labels(priv, attr, label_store):
...
################################################################################
def _init_patched_dependencies_labels(priv, rctx, attr, label_store):
...
################################################################################
def _maybe_npmrc(priv, rctx, rctx_name, attr, label_store, key):
    is_windows = repo_utils.is_windows(rctx)
    if is_windows:
        # TODO(windows): utils.exists is not yet support on Windows
        return
...
""".format(npmrc = npmrc_label, rctx_name = rctx_name))
...
def _copy_unspecified_input_files(priv, rctx, rctx_name, attr, label_store):
    pnpm_lock_label = label_store.label("pnpm_lock")
...
################################################################################
def _write_action_cache(priv, rctx, rctx_name, label_store):
    contents = [
        "# @generated",
        "# Input hashes for repository rule npm_translate_lock(name = \"{}\", pnpm_lock = \"{}\").".format(helpers.to_apparent_repo_name(rctx_name), str(label_store.label("pnpm_lock"))),
...
def _new(rctx, rctx_name, attr):
    ...
    _init(priv, rctx, rctx_name, attr, label_store)
    ...
    return struct(
        ...
        write_action_cache = lambda: _write_action_cache(priv, rctx, rctx_name, label_store),
        reload_lockfile = lambda: _reload_lockfile(priv, rctx, rctx_name, attr, label_store),
    )
