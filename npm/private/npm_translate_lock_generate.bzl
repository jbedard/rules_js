...
def generate_repository_files(rctx, rctx_name, attr, pnpm_lock_label, importers, packages, patched_dependencies, root_package, default_registry, npm_registries, npm_auth, link_workspace, bzlmod, bzlmod_npm_import_lambda):
    # empty line after bzl docstring since buildifier expects this if this file is vendored in
    generated_by_prefix = "\"\"\"@generated by npm_translate_lock(name = \"{}\", pnpm_lock = \"{}\")\"\"\"\n".format(helpers.to_apparent_repo_name(rctx_name), str(pnpm_lock_label))

    npm_imports = helpers.get_npm_imports(importers, packages, patched_dependencies, root_package, rctx_name, attr, attr.lifecycle_hooks, attr.lifecycle_hooks_execution_requirements, attr.lifecycle_hooks_use_default_shell_env, npm_registries, default_registry, npm_auth)
...
sh_binary(
    ...
    srcs = ["@aspect_rules_js//npm/private:noop.sh"],
            ...
            "exports_files({})".format(starlark_codegen_utils.to_list_attr([
                attr.defs_bzl_filename,
                attr.repositories_bzl_filename if not bzlmod else None,
            ])),
        ],
    }
...
""".format(
            defs_bzl_file = "@{}//:{}".format(rctx_name, attr.defs_bzl_filename),
    ...
    helpers.check_for_conflicting_public_links(npm_imports, attr.public_hoist_packages)
    ...
    for (i, _import) in enumerate(npm_imports):
        if not bzlmod:
            repositories_bzl.append(_gen_npm_import(attr, _import, link_workspace))
        else:
            _bzlmod_npm_import(bzlmod_npm_import_lambda, _import, link_workspace)
    ...
    rctx_files[attr.defs_bzl_filename] = [
    ...
    for filename in rctx_files.keys():
        rctx_files[filename] = generated_by_prefix + "\n" + "\n".join(rctx_files[filename])

    if not bzlmod:
        for filename, contents in rctx_files.items():
            rctx.file(filename, contents)
    else:
        _bzlmod_repository_files(
            name = rctx_name,
            files = rctx_files,
        )

def _bzlmod_npm_import(_npm_import_rule, _import, link_workspace):
    _npm_import_rule(
        ...
        patch_args = _import.patch_args,
        lifecycle_hooks = _import.lifecycle_hooks,
        custom_postinstall = _import.custom_postinstall,
        lifecycle_hooks_env = _import.lifecycle_hooks_env,
        lifecycle_hooks_execution_requirements = _import.lifecycle_hooks_execution_requirements,
        bins = _import.bins,
        ...
        replace_package = _import.replace_package,
        lifecycle_hooks_use_default_shell_env = not (not (_import.lifecycle_hooks_use_default_shell_env and _import.lifecycle_hooks_use_default_shell_env[0] == "true")),
    )

def _bzlmod_repository_files_impl(rctx):
    for filename, contents in rctx.attr.files.items():
        print("WRITE: ", filename)

        rctx.file(filename, contents)

_bzlmod_repository_files = repository_rule(
    implementation = _bzlmod_repository_files_impl,
    attrs = {
        # "name": attr.label(),
...
def _gen_npm_import(attr, _import, link_workspace):
    maybe_integrity = ("""
    ...
    maybe_generate_bzl_library_targets = ("""
        generate_bzl_library_targets = True,""") if attr.generate_bzl_library_targets else ""
    maybe_commit = ("""
