...
def generate_repository_files(rctx, rctx_name, attrs, pnpm_lock_label, importers, packages, patched_dependencies, only_built_dependencies, root_package, default_registry, npm_registries, npm_auth, link_workspace, bzlmod):
    ...
    npm_imports = helpers.get_npm_imports(importers, packages, patched_dependencies, only_built_dependencies, root_package, rctx_name, attrs, attrs.lifecycle_hooks, attrs.lifecycle_hooks_execution_requirements, attrs.lifecycle_hooks_use_default_shell_env, npm_registries, default_registry, npm_auth)
...
sh_binary(
    ...
    srcs = ["@aspect_rules_js//npm/private:noop.sh"],
            ...
            "exports_files({})".format(starlark_codegen_utils.to_list_attr(
                [attrs.defs_bzl_filename] + [] if bzlmod else [attrs.repositories_bzl_filename],
            )),
        ],
    }
...
""".format(
            defs_bzl_file = "@{}//:{}".format(rctx_name, attrs.defs_bzl_filename),
    ...
    for (i, _import) in enumerate(npm_imports):
        ...
        for link_package in _import.link_packages.keys():
            ...
            rctx_files[resolved_json_file_path] = [json.encode({
            ...
            if _import.package_info.get("has_bin"):
                if attrs.generate_bzl_library_targets:
    ...
    for fp_link in fp_links.values():
        ...
        fp_target = "//{}:{}".format(
            fp_path,
            attrs.npm_package_target_name.replace("{dirname}", paths.basename(fp_path)),
        )
        ...
        package_visibility, _ = helpers.gather_values_from_matching_names(True, attrs.package_visibility, "*", fp_package)
    ...
    rctx_files[attrs.defs_bzl_filename] = [
    ...
    for filename, contents in attrs.additional_file_contents.items():
    ...
    if not bzlmod:
        rctx_files[attrs.repositories_bzl_filename] = _generate_repositories(
            rctx,
            npm_imports,
            pnpm_lock_label,
            link_workspace,
    ...
    for filename, contents in rctx_files.items():
        r[filename] = generated_by_prefix + "\n" + "\n".join(contents)
