...
def npm_imported_package_store(name):
    ...
    if not is_root:
        msg = "No store links in bazel package '%s' for npm package npm package rollup@2.14.0. This is neither the root package nor a link package of this package." % bazel_package
        fail(msg)
    ...
    deps = {
        ":.aspect_rules_js/{}/fsevents@fsevents+2.3.3/pkg".format(link_root_name): "fsevents",
        ":.aspect_rules_js/{}/fsevents@2.3.3/pkg".format(link_root_name): "fsevents",
        ":.aspect_rules_js/{}/rollup@2.14.0/pkg".format(link_root_name): "rollup",
    }
    ref_deps = {
        ":.aspect_rules_js/{}/fsevents@fsevents+2.3.3/ref".format(link_root_name): "fsevents",
    }
...
def npm_link_imported_package_store(name):
    ...
    _npm_link_package_store(
        name = name,
        package = link_alias,
        src = "//<LOCKVERSION>:{}".format(store_target_name),
        visibility = ["//visibility:public"],
        tags = ["manual"],
    )
...
def npm_link_imported_package(
    ...
    if fail_if_no_link and not is_root and not link:
        msg = "Nothing to link in bazel package '%s' for npm package npm package rollup@2.14.0. This is neither the root package nor a link package of this package." % bazel_package
        fail(msg)
