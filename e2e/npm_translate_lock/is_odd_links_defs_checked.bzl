"@generated by @aspect_rules_js//npm/private:npm_import.bzl for npm package is-odd@3.0.1"
...
# Generated npm_package_store targets for npm package is-odd@3.0.1
# buildifier: disable=function-docstring
def npm_imported_package_store(name):
    root_package = ""
    is_root = native.package_name() == root_package
    if not is_root:
        msg = "No store links in bazel package '%s' for npm package npm package is-odd@3.0.1. This is neither the root package nor a link package of this package." % native.package_name()
        fail(msg)
    if not name.endswith("/is-odd"):
        msg = "name must end with one of '/is-odd' when linking the store in package 'is-odd'; recommended value is 'node_modules/is-odd'"
        fail(msg)
    link_root_name = name[:-len("/is-odd")]

    deps = {
        ":.aspect_rules_js/{}/is-number@6.0.0/pkg".format(link_root_name): "is-number",
        ":.aspect_rules_js/{}/is-odd@3.0.1/pkg".format(link_root_name): "is-odd",
    }
    lc_deps = {
        ":.aspect_rules_js/{}/is-number@6.0.0/pkg".format(link_root_name): "is-number",
        ":.aspect_rules_js/{}/is-odd@3.0.1/pkg_pre_lc_lite".format(link_root_name): "is-odd",
    }
    ref_deps = {
        ":.aspect_rules_js/{}/is-number@6.0.0/ref".format(link_root_name): "is-number",
    }

    has_lifecycle_build_target = False
    store_target_name = ".aspect_rules_js/{}/is-odd@3.0.1".format(link_root_name)
    ...
    _npm_package_store(
        ...
        src = "{}/pkg_lc".format(store_target_name) if has_lifecycle_build_target else "@npm//:npm__is-odd__3.0.1_source_directory",
        package = "is-odd",
        version = "3.0.1",
        dev = False,
        deps = ref_deps,
    ...
    _npm_package_store(
        ...
        src = None if True else "@npm//:npm__is-odd__3.0.1_source_directory",
        package = "is-odd",
        version = "3.0.1",
        dev = False,
        deps = deps,
    ...
    if has_lifecycle_build_target:
        ...
        _js_run_binary(
            ...
            args = [
                       "is-odd",
                       "../../../$(execpath @@npm__is-odd__3.0.1//:pkg)",
                       "../../../$(@D)",
                   ] +
                   select({
            ...
            out_dirs = ["node_modules/.aspect_rules_js/is-odd@3.0.1/node_modules/is-odd"],
            tags = ["manual"],
            execution_requirements = {
                "no-sandbox": "1",
            },
            mnemonic = "NpmLifecycleHook",
            progress_message = "Running lifecycle hooks on npm package is-odd@3.0.1",
            env = {},
        )
...
# Generated npm_package_store and npm_link_package_store targets for npm package is-odd@3.0.1
# buildifier: disable=function-docstring
def npm_link_imported_package_store(name):
    ...
    if not link_alias:
        msg = "name must end with one of '/{{ {link_aliases_comma_separated} }}' when called from package 'is-odd'; recommended value(s) are 'node_modules/{{ {link_aliases_comma_separated} }}'".format(link_aliases_comma_separated = ", ".join(link_aliases))
        fail(msg)
...
def npm_link_imported_package(
    ...
    if fail_if_no_link and not is_root and not link:
        msg = "Nothing to link in bazel package '%s' for npm package npm package is-odd@3.0.1. This is neither the root package nor a link package of this package." % native.package_name()
        fail(msg)
    ...
    if is_root:
        npm_imported_package_store("{}/is-odd".format(name))
